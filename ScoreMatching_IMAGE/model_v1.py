...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        emb = self.linear1(t)
        emb = self.act(emb)
        return self.linear2(emb)  # (B, embed_dim)

class ScoreNetImage(nn.Module):
    def __init__(self, img_channels=1, out_channels=2, time_emb_dim=128):  # 修改输出为2通道
        super(ScoreNetImage, self).__init__()
        self.time_mlp = TimeEmbedding(time_emb_dim)

        self.encoder = nn.Sequential(
            nn.Conv2d(2, 16, kernel_size=5, padding=2),  # input has 2 channels now,
            nn.ReLU(),
            nn.MaxPool2d(2),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(16, out_channels, kernel_size=3, padding=1)  # ✅ 正确：输出通道 = 2
        )
